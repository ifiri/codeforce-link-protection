...
class Repository {
    ...
    const TABLE_PARAMS = 'tlp_aliases_params';

    public function __construct(Contracts\IConfigRepository $ConfigRepository) {
        $this->ConfigRepository = $ConfigRepository;
    }

    public function get_by($alias_id) {
        $result = null;

        try {
            $result = $this->get_params_from_database_by($alias_id);
    ...
    private function get_params_from_database_by($alias_id) {
        global $wpdb;
        ...
        $table_params = $wpdb->prefix . self::TABLE_PARAMS;
        ...
        $link_data['rules'] = $wpdb->get_results(
            $wpdb->prepare(
                'SELECT param_name, param_value FROM ' . $table_params . ' WHERE alias_id = %d', $alias_id
            ), ARRAY_A
        );
    ...
    private function add_rules_to_database_by($link_id, Contracts\IRequest $Request) {
        ...
        foreach ($rules as $rule_name => $rule_params) {
            ...
            $rule_value = $this->format_rule_value($Request->$rule_name);
    ...
    private function format_rule_value($rule_value) {
